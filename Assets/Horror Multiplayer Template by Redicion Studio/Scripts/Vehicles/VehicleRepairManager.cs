...
{
    public class VehicleRepairManager : NetworkBehaviour
    {
        [SyncVar] public NetworkIdentity vehicle;
        ...
        private void Update()
        {
            if (vehicle != null && vehicleHealth == null)
            {
            ...
            {
                RpcUpdatePlayerRepairUI(currentRepairerPlayer, true, vehicleHealth.currentHealth);
            }
        }
        ...
        {
            _player.GetComponent<PlayerInteractionModule>().UpdateRepairUI(repairs, value, currentRepairerPlayer.GetComponent<Player>().username, "Vehicle");
        }
        ...
        void Repair()
        {
            if (vehicleHealth.currentHealth != 100)
            {
                if (!canBeRepaired)
                {
                    canBeRepaired = true;
                ...
                {
                    vehicleHealth.currentHealth += currentRepairerPlayer.GetComponent<CharacterManager>().itemCurrentlyInUse.GetComponent<ToolboxManager>().repairPerFrameValue * currentRepairerPlayer.GetComponent<CharacterManager>().repairSpeedMultiplier;
                }
            }
            else if (vehicleHealth.currentHealth == 100)
            {
                canBeRepaired = false;
                beingRepaired = false;
                currentRepairerPlayer = null;
            ...
            else if (currentRepairerPlayer.GetComponent<HunterAbilities>()._inFight)
            {
                ...
                currentRepairerPlayer = null;
                StopCoroutine(RepairCoroutine());
                RpcCancelRepair(false, false, true, true, false, false, false);
            }
        }
        ...
        void RpcCancelRepair(bool _canBeRepaired, bool _beingRepaired, bool _stopRepairCoroutine, bool _clearCurrentRepairerPlayer, bool setAnimatorTrigger, bool enableItemMesh, bool unBlockPlayer)
        {
            ...
            currentRepairerPlayer.GetComponent<PlayerInteractionModule>().UpdateRepairUI(false, 0, currentRepairerPlayer.GetComponent<Player>().username, "Vehicle");
