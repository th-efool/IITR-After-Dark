...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [HideInInspector] public float DefaultSprintSpeed = 4.4f;
        ...
        [SerializeField] private bool _blockCamera = false;
        ...
        public float staminaRegenerationSpeed = 2f;
        public UnityEngine.UI.Slider staminaSlider;
        public TMPro.TMP_Text sprintSpeedText;
        ...
        public void BlockPlayer(bool blockMovement, bool blockCamera)
        {
        ...
        private void Update()
        {
            if (PlayerPrefs.HasKey("CameraSensitivity"))
                Sensitivity = PlayerPrefs.GetFloat("CameraSensitivity");
            /*// Check for the '+' key (increasing move speed)
            if (Input.GetKeyDown(KeyCode.Equals) || Input.GetKeyDown(KeyCode.Plus) || Input.GetKeyDown(KeyCode.KeypadPlus))
            {
                SprintSpeed += 0.1f;
                Debug.Log("MoveSpeed increased to: " + SprintSpeed);
            }

            // Check for the '-' key (decreasing move speed)
            if (Input.GetKeyDown(KeyCode.Minus) || Input.GetKeyDown(KeyCode.KeypadMinus))
            {
                SprintSpeed -= 0.1f;
                Debug.Log("MoveSpeed decreased to: " + SprintSpeed);
            }
            if(GetComponent<Player>().isLocalPlayer)
            {
                sprintSpeedText.gameObject.SetActive(true);
            }
            ...
            if (RedicionStudio.BSystem.BSystemUI.Instance.Active || RedicionStudio.InventorySystem.PlayerInventoryModule.inMenu || this.GetComponent<RedicionStudio.InventorySystem.PlayerInventoryModule>().inShop || this.GetComponent<CharacterManager>().alive == false || !RoomManager._instance.MatchStarted)
            {
        ...
        private void UpdateStamina()
        {
            ...
            if (canSprint && _input.sprint && _currentStamina > 0 && !GetComponent<CharacterManager>().isHealing)
            {
                // Sprinting: Decrease stamina and adjust sprint speed based on stamina
                targetSpeed = _input.sprint ? SprintSpeed : MoveSpeed;
                _currentStamina -= characterManager.staminaConsumption * Time.deltaTime;
            ...
            else // Regenerate stamina when not sprinting
            {
                if(!_input.sprint)
                {
                    _currentStamina += (characterManager.staminaRegeneration * staminaRegenerationSpeed) * Time.deltaTime;
                    _currentStamina = Mathf.Clamp(_currentStamina, 0f, MaxStamina);
                }
        ...
        {
            // Update the UI Slider value based on current stamina
        ...
        private void Move()
        {
            ...
            if (_hasAnimator)
            {
                _animator.SetFloat(_animIDSpeed, _animationBlend * MovementMultiplier);
                _animator.SetFloat(_animIDMotionSpeed, inputMagnitude * MovementMultiplier);
            }
        }
